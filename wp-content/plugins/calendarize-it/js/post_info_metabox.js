	...
	jQuery(document).ready(function($){
			...
			drop:function(e,ui){	
				...
				if( $(item).hasClass('pinfo_quick_icon') ){
					...
					var o = {
						...
						'custom': 			_type=='custom'?$(item).data('post_extrainfo_custom'):'',
		...
		$('.rhc-extra-info-cell').each(function(i,inp){
			...
			$('.post_extrainfo_control .post_extrainfo_cell').each(function(j,control){
				...
				var _value = '';
				//--fill values
				$.each([['post_extrainfo_span','span'],['post_extrainfo_type','type'],['post_extrainfo_label','label'],['post_extrainfo_custom','custom'],['post_extrainfo_value','value'],['post_extrainfo_taxonomy','taxonomy'],['post_extrainfo_postmeta','postmeta']],function(i,s){
					if( _input_id==s[0] ){
		...
		$('.rhc-extra-info-cell .pinfo-save').on('click',function(e){
			...
			var o = {
				...
				'custom': 			_type=='custom'?p.find('.post_extrainfo_custom').val():'',
